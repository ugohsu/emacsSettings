...
(defun osu-jump-put-overlay ()
  (let ((olist (reverse osu-jump-overlay-list))
    ...
    (while olist
      ...
      ;; 行頭と行末が一致する場合
      (if (equal (overlay-start (car olist))
                 (overlay-end (car olist)))
          (overlay-put (car olist) 'after-string
                       (propertize
  ...
  (unwind-protect
      (progn
        (let ((wl (append (cdr (window-list))
              ...
              (c (read-char-exclusive "Input Char: "))
              olist)
          ;; loop
          (while wl
            ;; ウィンドウの選択
            ...
            ;; オーバーレイをデフォルトに
            (let ((ol (make-overlay (window-start)
                                    (window-end))))
              (overlay-put ol 'category 'osu-jump)
              (overlay-put ol 'face 'default))
            ;; オーバーレイプット
      ...
      (progn
        (let* ((temp-buffer-show-function 'switch-to-buffer)
          ...
          (while blfile
            ...
            (insert "\t\t")
            (insert (with-current-buffer (car bl)
            ...
            (cond
             ...
             ((equal (with-current-buffer (car bl)
                       (symbol-name major-mode))
                     "dired-mode")
              (overlay-put
               (make-overlay (line-beginning-position)
